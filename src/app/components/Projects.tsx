...
const Projects = () => {
  ...
  const [showWorkingOnModal, setShowWorkingOnModal] = useState<number | null>(null);
  ...
  return (
    <section className="py-20">
          ...
          {workingOn.map((project: Project, index: number) => (
            ...
            >
              ...
              <div className="p-6 relative z-10">
                ...
                <button
                  onClick={() => setShowWorkingOnModal(index)}
      ...
      <AnimatePresence>
        ...
        {/* Working On Project Detail Modal */}
        {showWorkingOnModal !== null && (
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
            onClick={() => setShowWorkingOnModal(null)}
          >
            <motion.div
              initial={{ y: 50 }}
              animate={{ y: 0 }}
            ...
            >
              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="md:w-1/2">
                  <Image
                    src={workingOn[showWorkingOnModal].image}
                    alt={workingOn[showWorkingOnModal].title}
                    width={400}
                    height={192}
                    className="w-full h-48 object-cover rounded-lg"
                  />
                ...
                <div className="md:w-1/2">
                  <h3 className="text-2xl font-bold mb-2 text-gray-800 dark:text-white">
                    {workingOn[showWorkingOnModal].title}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-300 mb-4">
                    {workingOn[showWorkingOnModal].description}
                  </p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {workingOn[showWorkingOnModal].technologies.map((tech: string, index: number) => (
                      <span
                        key={index}
                        className="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-100 rounded-full text-sm"
                      >
                  ...
                  <div className="flex space-x-4">
                    <a
                      href={workingOn[showWorkingOnModal].githubLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
                    >
                    ...
                    <a
                      href={workingOn[showWorkingOnModal].liveLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
                    >
